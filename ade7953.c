...
int ade7953WriteValue(ade7953_t * psADE7953, u16_t Reg, void * pV, i32_t I32) {
	...
	int Len = 0;
	while (Size--)
		caBuf[Len++] = (I32 >> (8 * Size)) & 0xFF;		// correct BE -> LE conversion
	int iRV = ade7953Write(psADE7953, Reg, caBuf);
	if (pV && iRV > erFAILURE)
		memcpy(pV, caBuf, Len);							// Update local memory store
	return iRV;
}
...
int ade7953ReportConfig(report_t * psR, ade7953_t * psADE7953) {
	...
	iRV += xReport(psR, "CONFIG\tx%04X\tCL=%d ZXE=%d ZXI=%d CRCen=%d SWrst=%d ZXlpf=%d ", psADE7953->config.val,
	...
	iRV += xReport(psR, "REVP_P=%d REVP_CF=%d PFMode=%d HPFen=%d IEB=%d IEA=%d\r\n",
	...
	iRV += xReport(psR, "CFMODE\tx%04X\tCF2dis=%d CF1dis=%d CF2sel=%d CF1sel=%d\r\n", psADE7953->cfmode.val,
	...
	iRV += xReport(psR, "ALTOUT\tx%04X\trevp=%X zxi=%X zx=%X\r\n", psADE7953->alt_out.val,
	...
	iRV += xReport(psR, "ACCMOD\tx%06X\tBvarnl=%d Bvanl=%d Bactnl=%d Avarnl=%d Avanl=%d Aactnl=%d Bvarsign=%d Avarsign=%d ", psADE7953->accmode.val,
	...
	iRV += xReport(psR, "Bapsign=%d Aapsign=%d Bvaacc=%d Avaacc=%d Bvaracc=%d Avaracc=%d Bwattacc=%d Awattacc=%d\r\n",
...
int ade7953ReportCalib(report_t * psR, ade7953_t * psADE7953) {
	...
	iRV += xReport(psR, "PGA_X\tV=%d/%d  IA=%d/%d  IB=%d/%d\r\n", psADE7953->pga.v, xlatPGA[psADE7953->pga.v],
...
int ade7953ReportIRQs(report_t * psR, ade7953_t * psADE7953) {
	...
	int iRV = xReport(psR, "IRQ_A\tEN=x%06X\tSTAT=x%06X", psADE7953->ie_a.val, psADE7953->is_a.val);

	const char caStat1[] = "\tCRC=%d RESET=%d SAG=%d CYCEND=%d WSMP=%d OV=%d ZXV=%d ZXTO=%d\r\n";
	iRV += xReport(psR, caStat1, psADE7953->is_a.CRC, psADE7953->is_a.RESET,
	...
	iRV += xReport(psR, caStat2, psADE7953->is_a.OIA, psADE7953->is_a.ZXIA, psADE7953->is_a.ZXTO_IA,
	...
	iRV += xReport(psR, caStat3, psADE7953->is_a.AP_NOLOADA, psADE7953->is_a.VAEOFA, psADE7953->is_a.VAREOFA,
	...
	iRV += xReport(psR, "IRQ_B\tEN=x%06X\tSTAT=x%06X\r\n", psADE7953->ie_b.val, psADE7953->is_b.val);

	iRV += xReport(psR, caStat2, psADE7953->is_b.OIB, psADE7953->is_b.ZXIB, psADE7953->is_b.ZXTO_IB,
		psADE7953->is_b.VARSIGN_B, psADE7953->is_b.APSIGN_B, psADE7953->is_b.VA_NOLOADB, psADE7953->is_b.VAR_NOLOADB);

	iRV += xReport(psR, caStat3, psADE7953->is_b.AP_NOLOADB, psADE7953->is_b.VAEOFB, psADE7953->is_b.VAREOFB,
